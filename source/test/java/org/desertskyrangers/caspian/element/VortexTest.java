...
public class VortexTest {
	...
	@Test
	void testVelocity() {
		assertThat( new Vortex( 0, 0, 1 ).velocity( 1, 0 ) ).isEqualTo( new double[]{ 1 / (Cfd.TWO_PI * 1), 0 } );
		assertThat( new Vortex( 0, 0, 5 ).velocity( 7, 0 ) ).isEqualTo( new double[]{ 5 / (Cfd.TWO_PI * 7), 0 } );

		assertThat( new Vortex( 0, 0, 5 ).velocity( 0, 7 )[ 0 ] ).isCloseTo( 0.0, Offset.offset( 1e-16 ) );
